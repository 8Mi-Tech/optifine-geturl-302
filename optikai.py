...
@app.route('/kfile/<path:file_name>')
...
def fetch_file(file_name):
    ...
    url = f"https://livzmc.net/optikai/downloadx?f={file_name}"
    response = httpx.get(url)
    text = response.text
    download_pattern = r'<a rel="nofollow" href="([^"]+)" class="hover:underline text-lg">Download</a>'
    raw_link = re.search(download_pattern, text, re.DOTALL)
    
    if not raw_link:
        return jsonify(error_response), 404
    
    download_link = raw_link.group(1)
